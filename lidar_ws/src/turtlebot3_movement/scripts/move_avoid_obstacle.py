...
def isObstacleTooClose(laser_scan_msg, min_view_deg, max_view_deg, dist_threshold):
    ...
    arr = laser_scan_msg.ranges
    arr_cleaned = [x for x in arr if not math.isnan(x)]
    deg_left = arr_cleaned[0:max_view_deg]
    deg_right = arr_cleaned[len(arr_cleaned)+min_view_idx:359] + [arr_cleaned[len(arr_cleaned)-1]]
...
def scan_cb(msg):
    ...
    obstacle, left_obs, range_obs, ort_obs = isObstacleTooClose(msg, min_deg, max_deg, 0.5) #optimized
